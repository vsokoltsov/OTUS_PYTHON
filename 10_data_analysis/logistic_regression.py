...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in xrange(num_iters):
            ...
            item_num = np.random.choice(num_train, size=batch_size, replace=False)
            X_batch = X[item_num]
            y_batch = y[item_num]
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        y_eq_1 = self.hypothes(X.dot(self.w))
        y_eq_0 = 1.0 - y_eq_1

        # Took this from HW explanation
        y_proba = np.vstack((y_eq_0, y_eq_1)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        m = X_batch.shape[0]
        h = self.hypothes(X_batch.dot(self.w))
        loss = (1.0 / m) * (-y_batch.dot(np.log(h)) - (1 - y_batch).dot(np.log(1 - h)))
        dw = (1.0 / m) * (X_batch.T.dot(h - y_batch))
        ...
        loss += (reg / 2.0 * m) * self.w[:-1].dot(self.w[:-1])
        dw_reg = (reg / m) * self.w
        dw_reg[-1] = 0
        dw += dw_reg
